 ...
 */
definition(
	name: "Reciva Radio (Connect)",
	namespace: "Petezah",
	author: "Peter Dunshee",
	description: "Allows you to control your Reciva Radio from the SmartThings app. Perform basic functions like play, pause, stop, change track, and check artist and song name from the Things screen.",
...
preferences {
	page(name:"recivaDiscovery", title:"Reciva Radio Device Setup", content:"recivaDiscovery", refreshTimeout:5)
}
//PAGES
def recivaDiscovery()
{
	if(canInstallLabs())
	{
		...
		return dynamicPage(name:"recivaDiscovery", title:"Discovery Started!", nextPage:"", refreshInterval:refreshInterval, install:true, uninstall: true) {
			section("Please wait while we discover your Reciva Radio. Discovery can take five minutes or more, so sit back and relax! Select your device below once discovered.") {
				input "selectedSonos", "enum", required:false, title:"Select Radio (${numFound} found)", multiple:true, options:options
			}
		}
	}
	else
...
private discoverSonoses()
{
    ...
    log.trace "Trying to discover radios"
...
def addSonos() {
	...
	selectedSonos.each { dni ->
		...
		if(!d) {
			...
			log.trace "dni = $dni"
			d = addChildDevice("smartthings", "Reciva Radio", dni, newPlayer?.value.hub, [label:"${newPlayer?.value.name} Reciva Radio"])
...
def locationHandler(evt) {
    ...
    def ssdpTermFromEvt = parsedEvent?.ssdpTerm
    log.trace "Got parsedEvent: $parsedEvent"
    log.trace "got ssdpTerm: $ssdpTermFromEvt"
	if (parsedEvent?.ssdpTerm?.contains("urn:schemas-upnp-org:device:MediaRenderer:1"))
	{ //SSDP DISCOVERY EVENTS

		log.trace "media renderer found"
		def sonoses = getSonosPlayer()

		if (!(sonoses."${parsedEvent.ssdpUSN.toString()}"))
		{ //sonos does not exist
        	log.trace "got a new radio device"
	...
	{ // SONOS RESPONSES
		...
		{ // description.xml response (application/xml)
        	log.debug "Got a description.xml: $bodyString"
...
private def parseEventMessage(String description) {
	log.trace "Got event message: $description"
